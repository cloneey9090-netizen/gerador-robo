...
def baixar_lista_do_drive(link_drive, log_func=print):
    log_func("A ligar ao Google Drive...")
    try:
        resposta = requests.get(link_drive, timeout=15)
        if resposta.status_code == 200:
            log_func("Lista do Drive descarregada com sucesso!")
            return resposta.text.splitlines()
        else:
            log_func(f"Erro ao aceder ao Drive (Estado: {resposta.status_code})")
            return []
    except Exception as e:
        log_func(f"Falha na ligação com o Drive: {e}")
        return []
...
def caçar_links_iptv_org(link_mestre, log_func=print):
    ...
    try:
        ...
        if resposta.status_code != 200:
            log_func(f"❌ Erro ao aceder à fonte mestre (Estado: {resposta.status_code})")
            return links_finais
                    ...
                    if match_exato:
                        if idx + 1 < len(linhas):
                            ...
                            if link_candidato.startswith("http"):
                                log_func(f"[TESTANDO] Pulso de {alvo}...")
...
def executar_processamento(log_func=print, custom_drive=None, custom_mestre=None):
    ...
    if not conteudo_base and os.path.exists("lista.txt"):
        log_func("A usar a lista local de segurança.")
        with open("lista.txt", "r", encoding="utf-8") as f:
            conteudo_base = f.read().splitlines()

    if not conteudo_base:
        log_func("Sem dados para processar.")
        return
    
    novos_links = caçar_links_iptv_org(link_mestre_atual, log_func)
    ...
    for canal, dados in novos_links.items():
        if canal not in canais_encontrados_na_base:
            ...
            adicionados_novos += 1
            log_func(f"➕ [ADICIONADO] Novo canal: {canal}")
    ...
    log_func(f"Sincronização concluída! {adicionados_novos} novos canais adicionados. Salvo em: {caminho_arquivo_final}")
